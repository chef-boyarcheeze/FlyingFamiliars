...
public class FFItemClientExtension implements IClientItemExtensions
{
    public static final FFItemClientExtension INSTANCE = new FFItemClientExtension();
    ...
    public boolean applyForgeHandTransform(PoseStack poseStack, LocalPlayer player, HumanoidArm arm, ItemStack stack, float partialTicks, float equipProgress, float swingProgress)
    {
        ...
        {
            BaseWandEffect selectedWandEffect = item.getSelectedWandEffect(stack);
            ...
            if (selectedWandEffect != null)
            {
                ...
                if (player.isUsingItem() && player.getUseItemRemainingTicks() > 0 && player.getUsedItemHand() == hand)
                {
                    ...
                    switch (anim)
                    {
                        ...
                        case DRINK:
                            ...
                            if (percentage < 0.8f)
                            {
                                poseStack.translate(0, Mth.abs(Mth.cos(timeLeft / 4f * (float) Math.PI) * 0.1f), 0);
                            }
                        ...
                        case CROSSBOW:
                        {
                            ...
                            float charge = remainingTime / selectedWandEffect.getUseDurationMin();
                        ...
                        case SPEAR:
                            ...
                            float sCharge = sRemainingTime / selectedWandEffect.getUseDurationMin();
                            if (sCharge > 1) {
                                sCharge = 1;
                            }
                        ...
                        case BRUSH:
                            ...
                            applyItemArmTransform(poseStack, equipProgress, sideOffset);
                            float xRot = -15 + 75 * Mth.cos((1 - ((float) (player.getUseItemRemainingTicks() % 10) - partialTicks + 1) / 10) * 2 * (float) Math.PI);
                ...
                {
                    poseStack.translate(
                            sideOffset * -0.4f * Mth.sin(Mth.sqrt(swingProgress) * (float) Math.PI),
                            0.2f * Mth.sin(Mth.sqrt(swingProgress) * (float) Math.PI * 2),
                    ...
                    float rotation = Mth.sin(Mth.sqrt(swingProgress) * (float) Math.PI);
