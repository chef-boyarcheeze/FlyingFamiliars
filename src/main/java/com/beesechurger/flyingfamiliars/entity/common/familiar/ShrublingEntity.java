...
public class ShrublingEntity extends BaseFamiliarEntity
{
    protected static final float MAX_HEALTH = 12.00f;
    protected static final int FOLLOW_RANGE = 4;
    protected static final int VARIANTS = 8;

    public ShrublingEntity(EntityType<ShrublingEntity> entityType, Level level)
    {
    ...
    private void selectVariant(int variant)
    {
        if(!hasVariant())
        {
            switch(variant)
            {
                case 0 -> setVariant("oak");
                case 1 -> setVariant("birch");
                case 2 -> setVariant("spruce");
                case 3 -> setVariant("jungle");
                case 4 -> setVariant("dark_oak");
                case 5 -> setVariant("acacia");
                case 6 -> setVariant("mangrove");
                case 7 -> setVariant("cherry");
            }
        }
    }
    ...
    private <E extends GeoAnimatable> PlayState bodyController(AnimationState<E> event)
    {
        ...
        if(isFlying())
        {
            ...
            controller.setAnimation(RawAnimation.begin()
                    .thenLoop("animation.shrubling.body_flying"));
        }
        else if(!isFlying() && isMoving())
            controller.setAnimation(RawAnimation.begin()
                    .thenLoop("animation.shrubling.body_walking"));
        else if(isSitting())
            controller.setAnimation(RawAnimation.begin()
                    .thenLoop("animation.shrubling.body_sitting"));
        else
            controller.setAnimation(RawAnimation.begin()
                    .thenLoop("animation.shrubling.body_idle"));
