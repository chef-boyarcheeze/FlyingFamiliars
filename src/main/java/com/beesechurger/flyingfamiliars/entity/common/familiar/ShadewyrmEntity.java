...
public class ShadewyrmEntity extends BaseFamiliarEntity
{
    protected static final float MAX_HEALTH = 80.0f;
    protected static final int FOLLOW_RANGE = 12;
    protected static final int VARIANTS = 3;

    public ShadewyrmEntity(EntityType<ShadewyrmEntity> entityType, Level level)
    {
    ...
    private void selectVariant(int variant)
    {
        if(!hasVariant())
        {
            switch(variant)
            {
                case 0 -> setVariant("onyx");
                case 1 -> setVariant("alabaster");
                case 2 -> setVariant("olivine");
            }
        }
    }
...
/////////////////////////////////
    ...
    private <E extends GeoAnimatable> PlayState mouthController(AnimationState<E> event)
    {
        ...
        controller.setAnimation(RawAnimation.begin().thenLoop("animation.shadewyrm.mouth_idle"));
    ...
    private <E extends GeoAnimatable> PlayState bodyController(AnimationState<E> event)
    {
        ...
        if(isFlying())
            event.getController().setAnimation(RawAnimation.begin()
                    .thenLoop("animation.shadewyrm.body_flying"));
        else if(!isFlying() && isMoving())
            event.getController().setAnimation(RawAnimation.begin()
                    .thenLoop("animation.shadewyrm.body_walking"));
        else
            event.getController().setAnimation(RawAnimation.begin()
                    .thenLoop("animation.shadewyrm.body_idle"));
