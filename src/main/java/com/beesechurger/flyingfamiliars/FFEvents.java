...
public class FFEvents
{
	...
	public static class Client
	{
		...
		@SubscribeEvent
		public static void preRenderGui(RenderGuiOverlayEvent.Pre event)
		{
		...
		@SubscribeEvent
		public static void preInventoryMouseRelease(ScreenEvent.MouseButtonReleased.Pre event)
		{
		...
		@SubscribeEvent
		public static void preInventoryScrollWheel(ScreenEvent.MouseScrolled.Pre event)
		{
			if (event.getScreen() instanceof AbstractContainerScreen<?> containerScreen)
			{
				Player player = Minecraft.getInstance().player;
		...
		public static void onKeyInput(InputEvent.Key event)
		{
			...
			if (player != null)
			{
				...
				{
					if (FFKeys.WAND_EFFECT_SELECT_STATE.wasPressed())
					{
						WandEffectSelectionScreen.INSTANCE.open(stack);
					}
					else if (FFKeys.WAND_EFFECT_SELECT_STATE.wasReleased() && WandEffectSelectionScreen.INSTANCE.isActive())
					{
	...
	@Mod.EventBusSubscriber(modid = FlyingFamiliars.MOD_ID, bus = Mod.EventBusSubscriber.Bus.FORGE)
	public static class Common
	{
		...
		public static void onAttachCapabilitiesPlayer(AttachCapabilitiesEvent<Entity> event)
		{
			...
			{
				if (!event.getObject().getCapability(PantheonAffinityProvider.PANTHEON_AFFINITY).isPresent())
				{
					event.addCapability(new ResourceLocation(FlyingFamiliars.MOD_ID, PantheonAffinityProvider.PANTHEON_AFFINITY_DATA), new PantheonAffinityProvider());
				}
			}
		}
		...
		public static void onPlayerCloned(PlayerEvent.Clone event)
		{
			if (event.isWasDeath())
			{
				event.getOriginal().getCapability(PantheonAffinityProvider.PANTHEON_AFFINITY).ifPresent(oldStore -> {
					event.getEntity().getCapability(PantheonAffinityProvider.PANTHEON_AFFINITY).ifPresent(newStore -> {
						newStore = new PantheonAffinityProvider.PantheonAffinity(oldStore);
					});
				});
			}
		}
